...
def show_job_detail(machine, job_id, **opts):
    ...
    table.append(["Environment Variables", "\n".join(["{}={}".format(k, v) for k,v in state.envs.items()]) if state.envs else None])
    table.append(["Activate Script", "\n".join(state.activate_script) if state.activate_script else None])
    table.append(["Build Executed", state.build_executed])
    table.append(["Build Parameters", "\n".join(["{}={}".format(k, v) for k,v in state.build_params.items()]) if state.build_params else None])
    table.append(["Build Script", "\n".join(state.build_script) if state.build_script else None])
    table.append(["Run Parameters", "\n".join(["{}={}".format(k, v) for k,v in state.run_params.items()]) if state.run_params else None])
    table.append(["Run Script", "\n".join(state.run_script) if state.run_script else None])
